<template>
  <div>
    ...
    >
      <div class="addbtn" @click="clickAdd">添加实物商品</div>
    </div>
      ...
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column fixed="right" label="操作" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              v-if="scope.row.state === 2"
              @click="clickExchangeView(scope.row)"
        ...
        <el-row :gutter="22">
          ...
          <el-col :span="10">
            ...
            >
              <el-row :gutter="10">
                <el-col :span="11">
                  <el-input v-model="formData.color" style="width: 150px" />
                </el-col>
                <el-col :span="1">
                  <el-color-picker
    ...
    <el-dialog title="兑换名单" :visible.sync="exchangeDialogVisible" top="2vh" :before-close="cancelExchange" :close-on-click-modal="false">
<!--      <el-button type="primary" icon="el-icon-upload2" @click="exportEntityExchangeList" round>导 出</el-button>-->
      <el-table :data="exchangeList" border style="margin-top:10px; width: 100%">
      ...
      <div class="block">
        <el-pagination
          layout="total, prev, pager, next"
          @current-change="handleChangeExchangeViewPage"
          :page-size="exchangePageQuery.pageSize"
          :total="exchangeTotal"
        >
        </el-pagination>
      </div>
...
import {
  reqAddMarketGoods,
  reqDeleteMarketGoods, reqExportEntityExchangeList,
  reqGetEntityExchangeList,
...
export default {
  ...
  methods: {
    ...
    clickExchangeView(row) {
      this.exchangePageQuery.goodsId = row.id;
      this.getEntityExchangeList();
      this.exchangeDialogVisible = true;
    },
    async getEntityExchangeList() {
      let param = `pageIndex=${this.exchangePageQuery.pageIndex}&pageSize=${this.exchangePageQuery.pageSize}&goodsId=${this.exchangePageQuery.goodsId}`;
      const result = await reqGetEntityExchangeList(param);
      this.exchangeList = result.data.data.records;
      this.exchangeTotal = result.data.data.total;
    },
    handleChangeExchangeViewPage(pageIndex) {
      this.exchangePageQuery.pageIndex = pageIndex;
      this.getEntityExchangeList();
    },
    async exportEntityExchangeList() {
      // 发起请求
      // this.$axios.get('/export/goods/entityExchangeList?goodsId=' + this.exchangePageQuery.goodsId, {
      //   responseType: 'blob' // 指定响应数据类型为 Blob 对象
      // }).then(response => {
      //   // 处理 Blob 对象
